...
are commonly used in the field are provided (e.g. SpCas9 and AsCas12a).
...
CRISPR nucleases are examples of RNA-guided DNA endonucleases. They require two
...
sequence, called the **spacer** sequence. The latter can be thought of as a
variable binding motif that can be specified by designing corresponding gRNA
sequences.

In summary, the **spacer** sequence is used in the gRNA construct to guide
the CRISPR nuclease to the target **protospacer** sequence in the host genome. 
The spacer sequence does not contain a PAM sequence 
(which prevents the nuclease from cutting gRNAs) while the 
protospacer sequence in the host genome includes the PAM sequence.
...
The genomic locations stored in the IRanges represent the PAM site locations in 
