...
# gRNA design for Cas13d 

Designing gRNAs for RNA-targeting nucleases, such as CasRx (RfxCas13d), is a bit different as the target space is composed of mRNA sequences instead of a set of DNA sequences (such as a reference genome).
...
The PFS sequence (the equivalent of a PAM sequence for RNA-targeting nucleases) for CasRx is N, meaning that there is no specific PFS sequences prefered by CasRx. 
...
Then we can design gRNAs using the usual function `findSpacers`:
...
# gRNA design for base editor BE4max
