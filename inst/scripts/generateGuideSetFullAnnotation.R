...
vcf <- "~/crisprIndices/snps/dbsnp151.grch38/00-common_all_snps_only_maf1perc.vcf.gz"
...
gs <- addSpacerAlignmentsIterative(gs, aligner='bowtie', txObject=txObject,
                                   tssObject=tssObject, n_mismatches=3,
                                   bowtie_index=bowtie_index)
gs <- addOffTargetScores(gs)
gs <- addOnTargetScores(gs)
gs <- addPamScores(gs)
gs <- addRepeats(gs, gr.repeats=gr.repeats.hg38)
gs <- addRestrictionEnzymes(gs)
gs <- addGeneAnnotation(gs, txObject=txObject, ignore_introns=FALSE, addPfam=TRUE, mart_dataset=mart_dataset)
gs <- addTssAnnotation(gs, tssObject=tssObject)
